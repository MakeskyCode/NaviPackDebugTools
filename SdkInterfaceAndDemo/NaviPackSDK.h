...
#define NAVIPACK_API extern "C" __declspec(dllexport)
#else
#define NAVIPACK_API
#define DLLCALL
#endif
...
NAVIPACK_API int DLLCALL Open(int id, const char* name, int param);
...
/// @return 返回值小于0，表示失败，等于0 表示成功
NAVIPACK_API int  DLLCALL StartMapping(int id, int mappingMode = 0);
...
NAVIPACK_API int DLLCALL  StopMapping(int id, int save_flag = 0);

NAVIPACK_API int DLLCALL EnableMapUpdate(int id, bool bUpdate_flag = false);
/// 读取NaviPack中的地图列表
/// @param[in] id NaviPack对象ID

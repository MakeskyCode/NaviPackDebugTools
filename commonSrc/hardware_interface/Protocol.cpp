...
Protocol::Protocol(int32_t interface_id, uint32_t maxPackageSize)
{
	...
	mRxZipBuffer = new uint8_t[maxPackageSize];
	mTxZipBuffer = new uint8_t[maxPackageSize];
...
Protocol * Protocol::CreateProtocol(int32_t interface_id, ProtocolType type, uint32_t max_pakage_size)
{
	...
	case PT_BUFFER:
		...
		break;
	case PT_SERAL_PACKAGE:
//	case PT_SERAL_PACKAGE_AT_ONCE:
		return new SerialPackageProtocol(interface_id, max_pakage_size);
		break;
	default:
		break;
	}
